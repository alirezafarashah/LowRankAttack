...
def eval_transferability():
    ...
    ######
    perturbations = torch.load(args.perturbations_path)
    test_loss = 0
    test_acc = 0
    n = 0
    with torch.no_grad():
        for delta, batch_idx in tqdm(perturbations):
        ...
        test_acc = test_acc / n
        print(f"test loss on l2 delta: {test_loss}, test acc: {test_acc}")
        logger.info(f"test loss on l2 delta: {test_loss}, test acc: {test_acc}")
